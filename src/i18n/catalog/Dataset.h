...
SS_MSG(mask_dilate_help,
    EN("Grows every detected object outward before the mask is written, by this "
       "share of the object's own size -- a person 400 pixels tall gains about "
       "30 pixels at 15%. The outline the model draws hugs the object and "
    ...
    JA("マスクを書き出す前に、検出した対象をそれぞれ自身の大きさのこの割合だけ"
       "外側へ広げます。高さ400ピクセルの人物なら15%で約30ピクセルです。"
    ...
    DE("Vergrößert jedes erkannte Objekt nach außen, bevor die Maske "
       "geschrieben wird, um diesen Anteil seiner eigenen Größe -- eine 400 "
       "Pixel hohe Person gewinnt bei 15% etwa 30 Pixel. Die Kontur, die das "
    ...
    FR("Élargit chaque objet détecté avant que le masque ne soit écrit, de "
       "cette fraction de sa propre taille : une personne haute de 400 pixels "
       "gagne environ 30 pixels à 15%. Le contour que trace le modèle colle de "
    ...
    ES("Agranda cada objeto detectado antes de escribir la máscara, en esta "
       "fracción de su propio tamaño: una persona de 400 píxeles de alto gana "
       "unos 30 píxeles al 15%. El contorno que traza el modelo se ciñe "
    ...
    PT("Aumenta cada objeto detectado antes de a máscara ser escrita, nesta "
       "fração do seu próprio tamanho: uma pessoa com 400 pixels de altura "
       "ganha cerca de 30 pixels a 15%. O contorno que o modelo traça fica "
    ...
    IT("Ingrandisce ogni oggetto rilevato prima che la maschera venga scritta, "
       "di questa frazione della sua stessa dimensione: una persona alta 400 "
       "pixel guadagna circa 30 pixel al 15%. Il contorno che il modello "
    ...
    NL("Laat elk gedetecteerd object naar buiten groeien voordat het masker "
       "wordt geschreven, met dit deel van zijn eigen grootte: een persoon van "
       "400 pixels hoog wint er bij 15% ongeveer 30 bij. De omtrek die het "
    ...
    RU("Расширяет каждый найденный объект наружу перед записью маски -- на эту "
       "долю его собственного размера: человек высотой 400 пикселей при 15% "
       "прибавляет около 30. Контур, который рисует модель, прилегает слишком "
    ...
    TR("Maske yazılmadan önce algılanan her nesneyi kendi boyutunun bu oranı "
       "kadar dışa doğru büyütür: 400 piksel boyundaki bir kişi %15'te yaklaşık "
       "30 piksel kazanır. Modelin çizdiği sınır nesneye fazla yapışıktır ve "
       "nesnenin renginden bir kenar bırakır; yeniden oluşturma da bunu sahnenin "
       "parçası olarak öğrenir. Piksel sayısı değil oran olduğu için uzaktaki "
       "bir şey yakındakine göre orantılı olarak daha küçük pay alır. %0 sınırı "
       "modelin çizdiği gibi yazar."));
