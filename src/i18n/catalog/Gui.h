...
SS_MSG(snapshot_export_help,
    EN("Save the current model as dataset-name_YYYYMMDD_HHMMSS.ply in the dataset root, using the displayed center and rotation. Keeps scene size, the live model and original checkpoints. Also works while paused or after training."),
    JA("表示中の中心と回転で、データセット直下に dataset-name_YYYYMMDD_HHMMSS.ply として保存します。大きさ、学習モデル、元のチェックポイントは保持します。一時停止中や学習後も使えます。"),
    ZH_HANS("按当前中心和旋转，将此刻模型保存为 dataset名_YYYYMMDD_HHMMSS.ply，直接放在数据集根目录。保持场景尺寸，不改动训练模型和原始检查点。暂停时、训练结束后也可导出。"),
    ZH_HANT("按目前中心與旋轉，將此刻模型儲存為 dataset名_YYYYMMDD_HHMMSS.ply，直接放在資料集根目錄。保留場景尺寸，不更動訓練模型和原始檢查點。暫停時、訓練結束後也可匯出。"),
    KO("현재 중심과 회전으로 데이터셋 루트에 dataset-name_YYYYMMDD_HHMMSS.ply 파일을 저장합니다. 크기, 학습 모델, 원본 체크포인트는 유지됩니다. 일시 중지 또는 학습 완료 후에도 가능합니다."),
    DE("Das Modell mit angezeigtem Zentrum und Drehung als dataset-name_YYYYMMDD_HHMMSS.ply im Datensatzordner speichern. Größe, Trainingsmodell und Checkpoints bleiben erhalten. Auch pausiert oder nach dem Training verfügbar."),
    FR("Enregistrer le modèle avec le centre et la rotation affichés sous dataset-name_YYYYMMDD_HHMMSS.ply à la racine du jeu de données. Taille, modèle actif et checkpoints conservés. Disponible en pause et après l'entraînement."),
    ES("Guarda el modelo con el centro y rotación mostrados como dataset-name_YYYYMMDD_HHMMSS.ply en la raíz del conjunto de datos. Conserva tamaño, modelo activo y checkpoints. Funciona en pausa y al terminar."),
    PT("Salva o modelo com centro e rotação exibidos como dataset-name_YYYYMMDD_HHMMSS.ply na raiz do conjunto de dados. Preserva tamanho, modelo ativo e checkpoints. Funciona em pausa e após o treino."),
    IT("Salva il modello con centro e rotazione mostrati come dataset-name_YYYYMMDD_HHMMSS.ply nella cartella del dataset. Mantiene dimensioni, modello attivo e checkpoint. Disponibile in pausa e dopo l'addestramento."),
    NL("Sla het model met het getoonde centrum en de rotatie op als dataset-name_YYYYMMDD_HHMMSS.ply in de datasetmap. Grootte, actief model en checkpoints blijven behouden. Ook tijdens pauze en na training."),
    RU("Сохранить модель с выбранным центром и поворотом как dataset-name_YYYYMMDD_HHMMSS.ply в корне набора данных. Размер, обучаемая модель и исходные чекпойнты сохраняются. Доступно на паузе и после обучения."),
    TR("Modeli gösterilen merkez ve dönüşle veri kümesinin köküne dataset-name_YYYYMMDD_HHMMSS.ply olarak kaydeder. Boyut, etkin model ve kontrol noktaları korunur. Duraklatıldığında ve eğitim sonrasında da kullanılabilir."));
